...
export default function UserPopover({ isOpen, onClose, onOpenSettings, onOpenProfile, triggerRect }: Props) {
  ...
  return createPortal(
    <div 
      ref={popoverRef} 
      style={style} 
      className="bg-background-secondary border border-background-tertiary rounded-md shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-150 font-sans select-none"
    >
      {/* BANNIÈRE */}
      <div className="h-20 w-full bg-background-tertiary"></div>
      ...
      <div className="px-4 relative pb-3">
        ...
        <div className="absolute -top-12 left-4">
            ...
            >
                <div className="w-20 h-20 rounded-full bg-background-tertiary flex items-center justify-center text-text-header font-bold text-2xl overflow-hidden ring-[6px] ring-background-secondary transition-opacity group-hover:opacity-90">
                    {user.avatarUrl ? (
                        <img src={user.avatarUrl} className="w-full h-full object-cover" alt="Avatar" />
                    ) : (
                        <img src="/default_avatar.png" className="w-full h-full object-cover" alt="Avatar" />
                    )}
                </div>
                <div className="absolute bottom-1 right-1 w-6 h-6 bg-status-green border-[5px] border-background-secondary rounded-full"></div>
        ...
        <div className="mt-10 mb-4 pl-1">
            ...
            >
                <div className="flex flex-col">
                    <span className="font-bold text-xl text-text-header leading-tight">{user.username}</span>
                    <span className="text-xs text-text-muted font-medium">#{user.discriminator}</span>
                </div>
                ...
                <div className="opacity-0 group-hover:opacity-100 transition-opacity ml-auto mr-2">
                    {copySuccess ? (
                        ...
                        <svg className="w-4 h-4 text-text-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                    )}
                </div>
            </div>
        </div>
        ...
        {/* MENU ACTIONS */}
        <div className="space-y-1 border-t border-background-tertiary pt-2 mt-2">
            <button 
                onClick={() => { onOpenSettings(); onClose(); }}
                className="w-full text-left px-3 py-2 rounded-sm hover:bg-brand text-text-normal hover:text-white text-sm font-medium transition-colors flex items-center gap-3 group"
            >
            ...
            <div className="h-px bg-background-modifier-accent my-1 mx-2"></div>
