...
const ChannelsSkeleton = () => (
  <div className="px-2 pt-4 space-y-6 animate-pulse opacity-50">
      {[1, 2, 3].map((i) => (
          <div key={i}>
              ...
              <div className="space-y-1 pl-2">
                  <div className="h-8 w-full bg-zinc-700/30 rounded"></div>
                  <div className="h-8 w-full bg-zinc-700/30 rounded"></div>
                  <div className="h-8 w-full bg-zinc-700/30 rounded"></div>
              </div>
          </div>
...
export default function ServerChannels({ 
  ...
  const handleGlobalCreateChannel = () => {
      ...
      if (firstCat) setCreateChannelCategoryId(firstCat.id);
    ...
    <div className="flex flex-col h-full bg-[#2b2d31] select-none font-sans" onContextMenu={handleContextMenuGlobal}>
        ...
        {contextMenu && (
            <ContextMenu position={contextMenu} onClose={() => setContextMenu(null)}>
                 {contextMenu.type === 'GLOBAL' && ( <> <ContextMenuItem label="Créer un salon" onClick={handleGlobalCreateChannel} icon={Icons.Add} /> <ContextMenuSeparator /> <ContextMenuItem label="Créer une catégorie" onClick={() => { setIsCreateCategoryOpen(true); setContextMenu(null); }} icon={Icons.Folder} /> </> )}
                 {contextMenu.type === 'CHANNEL' && ( <> <ContextMenuItem label="Modifier le salon" onClick={() => { setEditingChannel(contextMenu.data); setContextMenu(null); }} icon={Icons.Edit} /> <ContextMenuSeparator /> <ContextMenuItem label="Supprimer le salon" onClick={handleDeleteChannelContext} variant="danger" icon={Icons.Trash} /> </> )}
                 {contextMenu.type === 'CATEGORY' && ( <> <ContextMenuItem label="Créer un salon" onClick={() => { setCreateChannelCategoryId(contextMenu.data.id); setContextMenu(null); }} icon={Icons.Add} /> <ContextMenuSeparator /> <ContextMenuItem label="Modifier la catégorie" onClick={() => { setEditingCategory(contextMenu.data); setContextMenu(null); }} icon={Icons.Edit} /> <ContextMenuItem label="Supprimer la catégorie" onClick={handleDeleteCategoryContext} variant="danger" icon={Icons.Trash} /> </> )}
            </ContextMenu>
        )}
        ...
        <ChannelModal isOpen={!!editingChannel} channel={editingChannel} onClose={() => setEditingChannel(null)} />
        <ChannelModal isOpen={!!createChannelCategoryId} categoryId={createChannelCategoryId || undefined} onClose={() => setCreateChannelCategoryId(null)} onSuccess={() => setCreateChannelCategoryId(null)} />
