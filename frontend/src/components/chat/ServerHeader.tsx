  ...
  return (
    <>
      <div className="relative shadow-sm border-b border-slate-900 z-50">
        <div 
            className="h-12 px-4 hover:bg-background-modifier-hover transition cursor-pointer flex items-center justify-between"
            onClick={() => setIsMenuOpen(!isMenuOpen)}
        >
            <h1 className="font-bold text-text-header truncate max-w-[150px]">{server.name}</h1>
            {isMenuOpen ? <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-text-header"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg> : <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-text-header"><path d="m6 9 6 6 6-6"/></svg>}
        </div>

        {isMenuOpen && (
            <>
                <div className="fixed inset-0 z-40 cursor-default" onClick={() => setIsMenuOpen(false)}></div>
                <div className="absolute top-14 left-2 w-56 bg-background-floating rounded-md shadow-xl p-1.5 z-50 animate-in fade-in zoom-in-95 duration-100 border border-background-tertiary">
                    {hasCreateInvite && (
                    ...
                    {hasManageServer && (
                        <div onClick={() => { onOpenSettings(); setIsMenuOpen(false); }} className="flex justify-between items-center px-2 py-2 rounded-sm hover:bg-brand text-text-muted hover:text-white cursor-pointer">
                    ...
                    {hasCreateChannel && (
                        <div onClick={() => { onCreateChannel(); setIsMenuOpen(false); }} className="flex justify-between items-center px-2 py-2 rounded-sm hover:bg-brand text-text-muted hover:text-white cursor-pointer mb-1">
                    ...
                    <div className="h-[1px] bg-background-modifier-accent my-1 mx-1"></div>
