...
export default function VideoRoom({ channel }: Props) {
  ...
  // Refs pour l'analyse audio (évite les re-renders)
    ...
    const init = async () => {
      try {
        ...
        if (userVideo.current) {
          ...
          userVideo.current.volume = 0; // Évite l'écho local (larsen)
        }
        ...
        alert("Accès micro/caméra refusé.");
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#111214] h-full relative overflow-hidden">
      ...
      <div className="h-20 bg-[#1e1f22] border-t border-[#2b2d31] flex items-center justify-center gap-6 shadow-[0_-5px_20px_rgba(0,0,0,0.3)] z-50">
            <button onClick={toggleVideo} className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${isVideoOff ? 'bg-white text-black' : 'bg-[#2b2d31] text-white hover:bg-[#383a40]'}`}>
                {isVideoOff ? <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M16 16v1a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h2m5.66 0H14a2 2 0 0 1 2 2v3.34l1 1L23 7v10"/><line x1="1" y1="1" x2="23" y2="23"/></svg> : <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>}
            </button>

            <button onClick={toggleMute} className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${isMuted ? 'bg-white text-black' : 'bg-[#2b2d31] text-white hover:bg-[#383a40]'}`}>
            ...
            <button onClick={handleLeave} className="w-14 h-12 rounded-full bg-status-danger text-white hover:bg-red-600 transition flex items-center justify-center">
