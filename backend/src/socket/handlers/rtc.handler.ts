...
export const registerRtcHandlers = (io: Server, socket: AuthenticatedSocket) => {
    ...
    socket.on("join_voice_channel", (roomId: string) => {
        console.log(`🎤 User ${socket.userId} joined voice room: ${roomId}`);
        
        if (usersInRoom[roomId]) {
            const length = usersInRoom[roomId].length;
            if (length >= 10) { // Augmenté à 10 pour tests
        ...
        console.log(`👥 Sending ${usersInThisRoom.length} existing users to new joiner`);
    ...
    socket.on("returning_signal", (payload) => {
        console.log(`↩️ Returning signal from ${socket.id} to ${payload.callerID}`);
    ...
    socket.on("leave_voice_channel", handleDisconnect);
